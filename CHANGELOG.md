...
- **Brush size preview** — pen, highlighter, and pixel eraser show a circle outline at the stylus/touch position on paper (radius from tool widths / `eraserRadius`)
...
- **Ink bounds** — `InkPainter` clips to the paper rectangle so stroke caps and legacy points no longer paint off-page; partial eraser also clamps to paper
- **Orientation change** — page portrait↔landscape scales and centers ink to fit the new canonical bounds; device rotation resets document/page transforms at ~1× so content stays on screen and centered
